...
function loadEmailSettings() {
  ...
  const statusEl = document.getElementById('firestore-settings-status');
  if (statusEl) statusEl.innerHTML = '<div style="color:var(--gray400);font-size:12px">⏳ Loading from Firestore...</div>';

  db.collection('adminSettings').doc('emailNotifications').get().then(doc => {
    const exists = doc.exists;
    const settings = exists ? doc.data() : {
      enabled: true,
      thresholds: [60, 30, 7],
      customThreshold: null,
      fromEmail: 'rafal.pietrzak.pl@gmail.com'
    ...
    // Update form fields
    const el60  = document.getElementById('alert-60');
    const el30  = document.getElementById('alert-30');
    const el7   = document.getElementById('alert-7');
    const elEn  = document.getElementById('alert-enabled');
    const elCus = document.getElementById('alert-custom');
    const elFr  = document.getElementById('alert-from-email');

    if (elEn)  elEn.checked  = settings.enabled !== false;
    if (el60)  el60.checked  = (settings.thresholds || [60,30,7]).includes(60);
    if (el30)  el30.checked  = (settings.thresholds || [60,30,7]).includes(30);
    if (el7)   el7.checked   = (settings.thresholds || [60,30,7]).includes(7);
    if (elCus) elCus.value   = settings.customThreshold || '';
    if (elFr)  elFr.value    = settings.fromEmail || 'rafal.pietrzak.pl@gmail.com';

    updateSchedulePreview(settings);

    // Render Firestore status box
    if (statusEl) {
      if (!exists) {
        statusEl.innerHTML = `<div style="color:var(--amber);font-size:12px">⚠️ No settings saved yet — using defaults. Click "Save settings" to store in Firestore.</div>`;
        return;
      }

      const thresholds = settings.thresholds || [60, 30, 7];
      const enabled    = settings.enabled !== false;
      const updatedAt  = settings.updatedAt ? new Date(settings.updatedAt).toLocaleString('en-GB') : '—';
      const updatedBy  = settings.updatedBy || '—';

      const rows = [
        { label: 'Status',     value: enabled ? '<span style="color:var(--green)">✅ Enabled</span>' : '<span style="color:var(--red)">⏸ Paused</span>' },
        { label: 'Alert days', value: `<span style="color:var(--sky)">${thresholds.sort((a,b)=>b-a).join(', ')} days before expiry</span>` },
        { label: 'From email', value: `<span style="color:var(--white)">${settings.fromEmail || '—'}</span>` },
        { label: 'Schedule',   value: '<span style="color:var(--white)">Daily at 08:00 Warsaw time</span>' },
        { label: 'Last saved', value: `<span style="color:var(--gray400)">${updatedAt}</span>` },
        { label: 'Saved by',   value: `<span style="color:var(--gray400)">${updatedBy}</span>` },
      ];

      statusEl.innerHTML = rows.map(r => `
        <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--navy3);font-size:12px">
          <span style="color:var(--gray400);min-width:100px">${r.label}</span>
          ${r.value}
        </div>`).join('');
    }
  }).catch(err => {
    if (statusEl) statusEl.innerHTML = `<div style="color:var(--red);font-size:12px">❌ Firestore error: ${err.message}</div>`;
  });
}
