  ...
  const cards = visible.map(p => {
    ...
    const trainRows = allTrainingPeriods.map(t => {
      ...
      return `<div class="pc-line">
        <span class="pc-line-label">↳ ${fmtShort(t.start)}→${fmtShort(t.end)} · ${rateLabel}</span>
...
function openModal(ds) {
  ...
  const training = ['training-start','training-end'];
  [...blue,...green,...amber,...training].forEach(t => {
    ...
    if (dayEvs.some(e=>e.type===t)) {
      ...
      if (training.includes(t)) btn.classList.add('active');
    }
  });
  ...
  if (type === 'training-end') {
    ...
    if (lastTraining && lastTraining.end === selectedDate) {
      ...
      document.getElementById('training-rate-sub').textContent =
        `Training: ${fmtShort(lastTraining.start)} → ${fmtShort(lastTraining.end)} (${days} days). Enter the daily rate:`;
...
function openExpModal(idx) {
  ...
  document.getElementById('exp-editing-idx
